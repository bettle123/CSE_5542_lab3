...
namespace Crawfis
{
	namespace Graphics 
	{
		...
		public:
			...
			{
				while (MatrixStack::conditionmatrix.size()>0) {
					MatrixStack::conditionmatrix.pop();
				}
			...
			// 11/29
			LookAtTransformSolution(std::string name, ISceneNode* subject, bool depth)
				: TransformNodeBase(name, subject)
			{
				this->eyePosition = Crawfis::Math::Vector3(10, 10, 10);
				//this->eyePosition = Crawfis::Math::Vector3(0, 2, 10);

				this->centerOfInterest = Crawfis::Math::Vector3(0, 0, 0);
				this->viewUp = Crawfis::Math::Vector3(0, 1, 0);	
				Matrix4 m;
				m.identity();
				MatrixStack::conditionmatrix.push(m);
			...
			virtual void Apply()
			{
				...
				modelView = MatrixStack::modelViewStack.top() * modelView;
					...
					if (MatrixStack::conditionmatrix.size() > 0) {
						modelView = MatrixStack::depthmodelView.top() * modelView;
						MatrixStack::depthmodelView.push(modelView);
					}
			...
			virtual void Unapply()
			{
				 ...
				 if (MatrixStack::conditionmatrix.size() > 0) {
					 MatrixStack::depthmodelView.pop();
				 }
			}
			//
