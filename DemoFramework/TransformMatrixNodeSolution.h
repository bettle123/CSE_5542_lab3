...
namespace Crawfis
{
	namespace Graphics 
	{
		...
		public:
			...
			virtual void Apply()
			{
					Matrix4 transform1 = MatrixStack::modelViewStack.top() * matrix;
					MatrixStack::modelViewStack.push(transform1);
				
					if (MatrixStack::conditionmatrix.size() > 0) {
						Matrix4 transform2 = MatrixStack::depthmodelView.top() * matrix;
						MatrixStack::depthmodelView.push(transform2);
					}
			}
			// 
